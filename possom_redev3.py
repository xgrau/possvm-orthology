...
def get_midpoint_outgroup_list(phy, exclude_this=None):
	...
	while current is not None:
		cdist += current.dist
		if cdist > (middist) and current is not exclude_this:
			break
		else:
			current = current.up
	print(nA, nB, current.get_leaf_names())
	return current

def get_midpoint_outgroup_list2(phy, init_node, exclude_this=None):
	"""
	Returns the node that divides the current tree into two distance-balanced
	partitions.
	"""
...
def parse_phylo(phy_fn, phy_id, do_root):
	...
	if do_root:
		...
		if itermidroot is not None:
			...
			for roi in range(niter):
				...
				# parse events
				evs, _, _, _ = parse_events(phy=phy_tmp)

				# store number of speciation events in this particular iteration
				num_evs_per_iter[roi] = len(evs)
			...
			phy_outgroup_ix = np.argmax(num_evs_per_iter)
			phy_outgroup = out_nod_per_iter[phy_outgroup_ix]

			# set outgroup
			# print(phy_outgroup_ix, phy_outgroup)
			logging.info("%s Best root found at iteration %i/%i" % (phy_id,phy_outgroup_ix,itermidroot))
	...
	logging.info("Number speciation events: %i" % (len(evs)) )
